...
class SweetMelodyGenerator:
    ...
    def generate_percussion(self, duration):
        ...
        for i in range(16):
            ...
            # Agregar un sonido de shaker en cada tiempo para mayor claridad
            shaker = self.generate_shaker(beat_frames)
            percussion_wave[start:end] += shaker

            # Agregar un suave golpe de caja en el tiempo 4 y 12
            if i == 4 or i == 12:
    ...
    def generate_snare(self, frames):
        ...
        envelope = np.exp(-np.linspace(0, 1, frames) * 8)
        snare = noise * envelope * 0.3  # Volumen ajustado para mayor claridad
        return snare

    def generate_shaker(self, frames):
        noise = np.random.uniform(-1, 1, frames)
        envelope = np.exp(-np.linspace(0, 1, frames) * 20)
        shaker = noise * envelope * 0.2
        return shaker

    def add_reverb(self, wave, decay=0.3, delay=0.04):
    ...
    def generate_bassline(self, duration):
        ...
        for chord_name in self.current_progression:
            ...
            freq = self.base_frequencies.get(f"{root_note}2", 65.41)
    ...
    def generate_section(self, duration):
        ...
        section_wave += chord_wave * 0.6
        section_wave += melody_wave * 0.9
        section_wave += percussion_wave * 0.4
